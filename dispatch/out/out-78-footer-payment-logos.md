...
- `php -l` iki dosyada 0 hata (PHP 8.5.10; bu turda PHP kodu değişmedi).
...
```
(PHP kodu değişmedi; çıktı mevcut durum kanıtıdır.)
...
0
```
(Tam çıktı: `scratch/out78/evidence-curl-staging.txt`.)
...
- Masaüstü 1440px: 4 chip **tek satır**; chip yüksekliği 30px (svg 18px); chip merkezleri alt bar merkezinden ≤1.5px sapma; ödeme bloğu x=620–961, "İstanbul" x=1265 → **tamamen solda**; yatay taşma **0px**.
...
```
Alternatif (scratch silinmişse): `git checkout fea2080 -- theme/sutre-child-v2/{footer.php,style.css,functions.php}` sonra dosyaları FTP ile aynı mutlak yola at (kalıcı kaynak git'tir). Production'a dokunulmadı → production geri alma gerekmez.
...
- Site geneli regresyon tester paketine kalır.

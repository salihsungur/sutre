...
- [ ] **P58 — Adres alan şeması Woo-native hizalama + Fatura/Gönderim ayrımı — `YAPILACAK` (21-09-2026, sahibin kararı; coder P57 run'ı BİTİNCE uygulanacak):** Sahibin manuel sipariş ekranı görseline (upload_20260921_230713_6.png) göre adres defteri alan seti Woo-native olacak: Ad, Soyad, Firma (ops), Adres satırı 1, Adres satırı 2 (ops), İlçe/Semt, Posta kodu, Ülke (TR kilit), Şehir, Telefon. **TC Kimlik KALDIRILIYOR (şimdilik), Mahalle/Köy de kaldırılıyor** (Woo ekranında yok). Adreslerim formu + sepet hesaplayıcı + Alan şeması tablosu güncellenir; meta keys doğrudan billing/shipping_* ile eşleşir. **Fatura/Gönderim ayrımı (sahibin 2. kararı):** teslimat ve fatura adresi iki ayrı akış; varsayılan = "Faturayı da aynı adrese gönder" işaretli; "Farklı fatura adresi" OPSİYONEL — işaretlenirse ayrı fatura adresi girilir (checkout + adres defteri entegrasyonu). ÖNCE P57 işlevsel bug'ları (POST akışları) çözülecek.
